...
class FrontController extends Controller
{
    ...
    public function compare(Request $request)
    {
        ...
        $tempSpecs = array_merge(
            $product1Specs ? $product1Specs : [],
            $product2Specs ? $product2Specs : [],
            $product3Specs ? $product3Specs : [],
        );

        foreach ($tempSpecs as $cat => $spec) {
            if (!array_key_exists($cat, $specsKeys)) {
                $specsKeys[$cat] = [];
            }
            foreach ($spec as $key => $value) {
                if (!in_array($key, $specsKeys[$cat])) {
                    $specsKeys[$cat][] = $key;
                }
            }
        }

        return view('frontend.compare', compact('product1', 'product2', 'product3', 'product1Specs', 'product2Specs', 'product3Specs', 'specsKeys'));
    }
